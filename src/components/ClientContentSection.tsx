...
export const ClientContentSection = ({
  ...
  // Build config from translations so it re-renders when language changes
  const contentTypeConfig = {
    live_maps: {
      name: t.contentSection.liveMapsName,
      icon: Map,
      color: 'text-blue-600',
      description: t.contentSection.liveMapsDesc
    },
    reports: {
      name: t.contentSection.reportsName,
      icon: FileText,
      color: 'text-green-600',
      description: t.contentSection.reportsDesc
    },
    hd_maps: {
      name: t.contentSection.hdMapsName,
      icon: Image,
      color: 'text-purple-600',
      description: t.contentSection.hdMapsDesc
    },
    '3d_models': {
      name: t.contentSection.modelsName,
      icon: Box,
      color: 'text-orange-600',
      description: t.contentSection.modelsDesc
    }
  };
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">{t.contentSection.loadingContent}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-8 px-4 sm:px-6 py-6 sm:py-8 bg-white/10 rounded-2xl mx-4 sm:mx-6 shadow-sm border border-white/20 backdrop-blur-md">
        ...
        {contentType === 'live_maps' && (
          ...
          >
            ...
            {showMapView ? t.contentSection.hideMapView : t.contentSection.showMapView}
          </Button>
        )}
      </div>
      {/* Map View */}
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex-1 min-w-[200px]">
              <div className="relative">
                ...
                <Input
                  placeholder={t.contentSection.searchPlaceholder}
                  value={searchTerm}
      ...
      <Card className="mx-[15px]">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t.contentSection.available} {config.name}
            <span className="text-sm font-normal text-muted-foreground">
              ({filteredFiles.length} {filteredFiles.length === 1 ? t.contentSection.fileSingular : t.contentSection.filePlural})
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {filteredFiles.length > 0 ? (
            ...
            <div className="text-center p-8 border-2 border-dashed border-border rounded-lg">
              ...
              <p className="text-muted-foreground">
                {searchTerm
                  ? t.contentSection.noFilesSearch
                  : t.contentSection.noFilesEmpty(config.name)}
              </p>
            </div>
          )}
        </CardContent>
